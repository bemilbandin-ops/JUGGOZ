        ...
        <aside aria-label="Effects">
          ...
          <div className="controls">
            <ControlGroup title="Tracking">
              ...
              {clubPreset && <PoiRange name="smoothing" label="Pose smoothing" value={poiControls.smoothing} min={40} max={92} suffix="%" onChange={updatePoiControl} />}
            </ControlGroup>

            {clubPreset ? (
              <>
                <ControlGroup title="Trail material">
                  <PoiRange name="lifetime" label="Lifetime" value={poiControls.lifetime} min={450} max={4000} step={50} suffix=" ms" onChange={updatePoiControl} />
                  <PoiRange name="brightness" label="Brightness" value={poiControls.brightness} min={30} max={100} suffix="%" onChange={updatePoiControl} />
...
function PatternControls({ preset, controls, update, loadImage, hasImage }: { preset: string; controls: PoiControls; update: (key: keyof PoiControls, value: number) => void; loadImage: (file?: File) => void; hasImage: boolean }) {
  if (preset === 'neon-rails') return <ControlGroup title="Rail geometry"><PoiRange name="railSeparation" label="Rail separation" value={controls.railSeparation} min={45} max={135} suffix="%" onChange={update} /><PoiRange name="crossbarFrequency" label="Crossbar spacing" value={controls.crossbarFrequency} min={12} max={60} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'prism-ribbon') return <ControlGroup title="Ribbon geometry"><PoiRange name="ribbonWidth" label="Ribbon width" value={controls.ribbonWidth} min={45} max={150} suffix="%" onChange={update} /><PoiRange name="cellDensity" label="Cell spacing" value={controls.cellDensity} min={10} max={42} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'chromatic-echoes') return <ControlGroup title="Echo geometry"><PoiRange name="echoCount" label="Echo count" value={controls.echoCount} min={6} max={12} onChange={update} /><PoiRange name="echoSpacing" label="Echo spacing" value={controls.echoSpacing} min={15} max={30} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'electric-comets') return <ControlGroup title="Particle geometry"><PoiRange name="branching" label="Branching" value={controls.branching} suffix="%" onChange={update} /><PoiRange name="turbulence" label="Turbulence" value={controls.turbulence} suffix="%" onChange={update} /></ControlGroup>;
  if (preset === 'kinetic-lattice') return <ControlGroup title="Lattice geometry"><PoiRange name="latticeDensity" label="Pose spacing" value={controls.latticeDensity} min={12} max={42} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'psychedelic-serpent') return <ControlGroup title="Serpent geometry"><PoiRange name="strandCount" label="Strand count" value={controls.strandCount} min={3} max={5} onChange={update} /><PoiRange name="waveAmplitude" label="Wave amplitude" value={controls.waveAmplitude} min={6} max={32} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'apex-shatter') return <ControlGroup title="Burst geometry"><PoiRange name="shardCount" label="Shard count" value={controls.shardCount} min={8} max={16} onChange={update} /><PoiRange name="shardSpread" label="Shard spread" value={controls.shardSpread} min={30} max={100} suffix="%" onChange={update} /></ControlGroup>;
  if (preset === 'pixel-mosaic') return <ControlGroup title="Mosaic geometry"><PoiRange name="tileSpacing" label="Tile spacing" value={controls.tileSpacing} min={15} max={36} suffix=" px" onChange={update} /><PoiRange name="shapeMix" label="Shape mix" value={controls.shapeMix} suffix="%" onChange={update} /></ControlGroup>;
  if (preset === 'radial-pov') return <ControlGroup title="Radial POV"><PoiRange name="radialSymmetry" label="Symmetry" value={controls.radialSymmetry} min={4} max={12} onChange={update} /><label className="file-button">{hasImage ? 'Replace POV image' : 'Upload POV image'}<input type="file" accept="image/*" onChange={(event) => loadImage(event.target.files?.[0])} /></label></ControlGroup>;
  if (preset === 'acid-blooms') return <ControlGroup title="Acid blooms"><PoiRange name="radialSymmetry" label="Petal count" value={controls.radialSymmetry} min={4} max={12} onChange={update} /><PoiRange name="tileSpacing" label="Bloom spacing" value={controls.tileSpacing} min={22} max={60} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'liquid-portal') return <ControlGroup title="Liquid portals"><PoiRange name="echoSpacing" label="Portal spacing" value={controls.echoSpacing} min={20} max={58} suffix=" px" onChange={update} /><PoiRange name="waveAmplitude" label="Ripple amount" value={controls.waveAmplitude} min={6} max={36} onChange={update} /></ControlGroup>;
  if (preset === 'kaleido-tunnel') return <ControlGroup title="Kaleido tunnel"><PoiRange name="radialSymmetry" label="Star points" value={controls.radialSymmetry} min={5} max={12} onChange={update} /><PoiRange name="latticeDensity" label="Wheel spacing" value={controls.latticeDensity} min={20} max={52} suffix=" px" onChange={update} /></ControlGroup>;
  if (preset === 'melting-rainbow') return <ControlGroup title="Melting rainbow"><PoiRange name="strandCount" label="Color bands" value={controls.strandCount} min={3} max={7} onChange={update} /><PoiRange name="waveAmplitude" label="Melt amount" value={controls.waveAmplitude} min={8} max={40} onChange={update} /></ControlGroup>;
  if (preset === 'hypno-eyes') return <ControlGroup title="Hypno eyes"><PoiRange name="echoSpacing" label="Eye spacing" value={controls.echoSpacing} min={24} max={64} suffix=" px" onChange={update} /><PoiRange name="shapeMix" label="Eye size" value={controls.shapeMix} min={30} max={100} suffix="%" onChange={update} /></ControlGroup>;
  return <ControlGroup title="Cosmic spores"><PoiRange name="tileSpacing" label="Colony spacing" value={controls.tileSpacing} min={18} max={50} suffix=" px" onChange={update} /><PoiRange name="branching" label="Orbit density" value={controls.branching} min={24} max={100} suffix="%" onChange={update} /><PoiRange name="waveAmplitude" label="Orbit radius" value={controls.waveAmplitude} min={8} max={36} onChange={update} /></ControlGroup>;
}
